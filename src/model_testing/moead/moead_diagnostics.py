"""
moead_diagnostics.py — Diagnóstico e Comparação do MOEA/D — TIAPOSE DSS

Compara o MOEA/D com o NSGA-II (baseline) no problema de otimização semanal.
...
# ---------------------------------------------------------------------------
# Path bootstrapping — permite executar diretamente (python moead_diagnostics.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
logging.basicConfig(
    level=logging.WARNING,   # silenciar logs dos algoritmos durante os N runs
    format="%(asctime)s [%(levelname)s] %(name)s — %(message)s",
    datefmt="%H:%M:%S",
)
log = logging.getLogger("TIAPOSE.MOEAD.Diagnostics")
...
POP_SIZE_NSGA2    = 100
N_PARTITIONS_MOEAD = 99     # 100 subproblemas
...
HV_REF_POINT = np.array([0.0, 210.0])  # [−lucro_min, staff_max]
...
def compute_hypervolume(pareto_F: np.ndarray, ref_point: np.ndarray) -> float:
    ...
    pareto_F: array (n, 2) com [−lucro, staff] (valores pymoo, minimização).
    ref_point: array (2,) com o pior ponto dominante.
    """
    from pymoo.indicators.hv import Hypervolume
    if pareto_F is None or len(pareto_F) == 0:
        return 0.0
    # Filtrar apenas soluções que dominam o ponto de referência
...
def run_single(algorithm: str, seed: int) -> dict:
    ...
    if algorithm == "MOEA/D":
        res = run_moead(
            ...
            n_partitions=N_PARTITIONS_MOEAD,
            n_max_gen=N_MAX_GEN,
            seed=seed,
            verbose=False,
...
def run_diagnostics():
    print("\n" + "=" * 70)
    print("  TIAPOSE — Diagnóstico MOEA/D vs NSGA-II")
    print(f"  Loja: {STORE.upper()} | {N_RUNS} corridas | max_gen={N_MAX_GEN}")
    print("=" * 70)

    results = {"MOEA/D": [], "NSGA-II": []}

    for algo in ["MOEA/D", "NSGA-II"]:
        print(f"\n  Algoritmo: {algo}")
    ...
    # --- Resumo estatístico ---
    print("\n" + "=" * 70)
    print(f"  {'Métrica':<28} | {'MOEA/D':>18} | {'NSGA-II':>18}")
    print("  " + "-" * 68)
    ...
    for metric, label in [("hv", "Hipervolume"), ("n_sol", "Nº Soluções Pareto"), ("elapsed", "Tempo (s)")]:
        ...
        tag_moead  = results["MOEA/D_" + metric][2]
        tag_nsga2  = results["NSGA-II_" + metric][2]
        print(f"  {label:<28} | {tag_moead:>18} | {tag_nsga2:>18}")
    ...
    for metric, label in [("best_lucro", "Lucro máx. (€)"), ("best_staff", "Staff @ lucro máx.")]:
        vals_m = results["MOEA/D_"  + metric]
        vals_n = results["NSGA-II_" + metric]
        tag_m = f"{np.mean(vals_m):.2f} ± {np.std(vals_m):.2f}"
        tag_n = f"{np.mean(vals_n):.2f} ± {np.std(vals_n):.2f}"
        print(f"  {label:<28} | {tag_m:>18} | {tag_n:>18}")

    print("=" * 70)

    # --- Guardar gráfico de comparação das fronteiras de Pareto ---
...
def _plot_pareto_comparison(results: dict):
    ...
    colors = {"MOEA/D": "#e05c2a", "NSGA-II": "#2a7ae0"}
    markers = {"MOEA/D": "o", "NSGA-II": "s"}

    for algo in ["MOEA/D", "NSGA-II"]:
        last = results[algo][-1]  # última corrida
        if len(last["lucro"]) == 0:
            continue
        ax.scatter(
    ...
    ax.set_title(f"Fronteira de Pareto — MOEA/D vs NSGA-II\nLoja: {STORE.upper()}", fontsize=12)
    ax.legend(fontsize=10)
    ax.grid(True, alpha=0.3)
    fig.tight_layout()

    out_dir = os.path.join(_SRC_DIR, "..", "results", "04_Model_Testing", "moead")
...
def _plot_hypervolume_comparison(results: dict):
    ...
    data   = [
        [r["hv"] for r in results["MOEA/D"]],
        [r["hv"] for r in results["NSGA-II"]],
    ]
    labels = ["MOEA/D", "NSGA-II"]
    colors = ["#e05c2a", "#2a7ae0"]
