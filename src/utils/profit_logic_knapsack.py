...
def optimize_allocation_wrapper(decision_vars, store, forecast_customers, forecast_is_weekend):
    """
    Wrapper para alocação conjunta (Mochila): Retorna Lucro, Unidades e Penalização.
    """
    weekly_plan = []
    total_units = 0
    penalizacao_semana = 0
    ...
    for i in range(7):
        ...
        # Penalização de staff (mesma lógica)
        staff_dia = hr_x_clean + hr_j_clean
        if not forecast_is_weekend[i]:
            if staff_dia > 8:
                penalizacao_semana += ((staff_dia - 8) * 1000)
        elif staff_dia > 12: # Adicionando limite de fim de semana para consistência
                penalizacao_semana += ((staff_dia - 12) * 1000)
        ...
        total_units += (metrics['units_x'] + metrics['units_j'])
    ...
    # f1: -Lucro, f2: Unidades, f3: Penalização
    return (-profit, float(total_units), penalizacao_semana)
